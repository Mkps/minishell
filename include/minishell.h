...
/*   Updated: 2023/09/26 22:13:14 by aloubier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
# define HEREDOC_EOF "minishell: warning: here-document delimited by end-of-file (wanted `"
...
/**		cmd_io.c		**/
int						handle_cmd_io(t_data *data, t_token *current_t,
							t_cmd *cmd);
...
int						output_err_ret(int return_value, char *msg,
							char *cmd_str);
...
int						var_expand_dollar(t_data *data, char **ret, int *i,
							t_token *token);

/**		var_utils.c		**/
char					*str_replace(char *src, int r_index, int n, char *str);
char					*str_replace_strs(char **src, int r_index, int n,
							char *str);
...
void					ft_lstadd_back_export(t_export **lst,
							t_export *new_elem);
...
int						export_key_exists(t_export *export_root,
							char *key_to_check);
...
char					*find_matching(char *search, char *src,
							char *(*function_ptr)(char *, char *, int),
							int mode);
...
char					*str_replace_free(char *src, int r_index, int n,
							char *str);
...
int						free_return(int return_value, void *ptr_1, void *ptr_2,
							void *ptr3);
void					multi_free(void *ptr_1, void *ptr_2, void *ptr_3,
							void *ptr_4);
